...
def exibir_mapa(estado: str = "Todos", tipo_restricao: str = "Todos", usina: str = "Todos"):
    ...
    mapa_gerado = gerar_mapa(dfEOL, estado=estado, tipo_restricao=tipo_restricao, usina=usina)
...
def obter_filtros():
    estados = ["Todos"] + list(dfEOL['nom_estado'].dropna().unique())
    restricoes = ["Todos"] + list(dfEOL['cod_razaorestricao'].dropna().unique())
...
def obter_usinas(estado: str):
    if estado == "Todos":
        usinas = ["Todos"] + list(dfEOL['nom_usina'].dropna().unique())
    else:
        usinas = ["Todos"] + list(dfEOL[dfEOL['nom_estado'] == estado]['nom_usina'].dropna().unique())
